...
func main() {
	...
	sconfig.AllowedAuthorizeTypes = osin.AllowedAuthorizeType{osin.CODE, osin.TOKEN, osin.CLIENT_CREDENTIALS}
	sconfig.AllowedAccessTypes = osin.AllowedAccessType{osin.AUTHORIZATION_CODE,
		osin.REFRESH_TOKEN, osin.PASSWORD, osin.CLIENT_CREDENTIALS, osin.ASSERTION}
	sconfig.AllowGetAccessRequest = true

	server := osin.NewServer(sconfig, rethinkdb.NewRethinkdbStorage("localhost:28015", "osin"))
	...
	http.HandleFunc("/app", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(fmt.Sprintf("<a href=\"/authorize?response_type=code&client_id=c178946c-bc21-4990-b918-fab2792e3cb9&state=xyz&scope=everything&redirect_uri=%s\">Code</a><br/>", url.QueryEscape("http://localhost:14000/appauth/code"))))
		w.Write([]byte(fmt.Sprintf("<a href=\"/authorize?response_type=token&client_id=c178946c-bc21-4990-b918-fab2792e3cb9&state=xyz&scope=everything&redirect_uri=%s\">Implict</a><br/>", url.QueryEscape("http://localhost:14000/appauth/token"))))
	...
	http.HandleFunc("/appauth/code", func(w http.ResponseWriter, r *http.Request) {
		...
		if code != "" {
			...
			aurl := fmt.Sprintf("/token?grant_type=authorization_code&client_id=c178946c-bc21-4990-b918-fab2792e3cb9&state=xyz&redirect_uri=%s&code=%s",
			...
			if r.Form.Get("doparse") == "1" {
				err := example.DownloadAccessToken(fmt.Sprintf("http://localhost:14000%s", aurl),
					&osin.BasicAuth{"c178946c-bc21-4990-b918-fab2792e3cb9", "aabbccdd"}, jr)
				if err != nil {
	...
	http.HandleFunc("/appauth/password", func(w http.ResponseWriter, r *http.Request) {
		...
		// doownload token
		err := example.DownloadAccessToken(fmt.Sprintf("http://localhost:14000%s", aurl),
			&osin.BasicAuth{Username: "c178946c-bc21-4990-b918-fab2792e3cb9", Password: "aabbccdd"}, jr)
		if err != nil {
			w.Write([]byte(err.Error()))
			w.Write([]byte("<br/>"))
		}
